const regions_1 = [
  ...
  let chosenRegion_1 = null;
  let chosenDistrict_1 = null;
  const region_1Select = document.querySelector("#regions_1");
  const district_1Select = document.querySelector("#districts_1");
  let output1 = "";
  regions_1.forEach((region_1) => {
    output1 += `<option value="${region_1.id}">${region_1.region_uz}</option>`;
  });
  region_1Select.innerHTML = output1;

  region_1Select.addEventListener("change", (e) => {
    const region_1 = e.target.value;
    chosenRegion_1 = region_1;
    const localDistricts_1 = districts_1.filter((d) => d.region_id == region_1);
    let output1 = "";
    localDistricts_1.forEach((district_1) => {
      output1 += `<option value="${district_1.id}">${district_1.region_uz}</option>`;
    });
    district_1Select.innerHTML = output1;
  });
  district_1Select.addEventListener("change", (e) => {
    chosenDistrict_1 = e.target.value;
    const region_1 = regions_1.filter((r) => r.id == chosenRegion_1)[0]
    const district_1 = districts_1.filter((r) => r.id == chosenDistrict_1)[0];
    const result = {
      region_1,
      district_1
    };
